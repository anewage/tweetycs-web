...
//function loom() {
  ...
  function loom(data) {
	  ...
	  data = d3.nest().key(outer).entries(data)
		  .sort(function(a,b) {  return catOrder.indexOf(a.key) - catOrder.indexOf(b.key);}) // so we can arrange them based on their data but we still have the problem of when it switches to the other side
